  ...
  function CreateStore(options: IOptions) {
    ...
    function loadToActionParams(loadOptions: any) {
      ...
      [
        ...
        if (el in loadOptions && isNotEmpty(loadOptions[el])) {
          params += `${el}=${JSON.stringify(loadOptions[el])}&`;
        }
      });
      if (customQueryParams) {
        for (const key in customQueryParams) {
          params += `${key}=${JSON.stringify(
            customQueryParams[key]
          )}&`.replaceAll('"', "");
        }
      }
    ...
    const store = new CustomStore({
      ...
      load: async (loadOptions) => {
        let result = await $axios.get(
          options.loadUrl + loadToActionParams(loadOptions)
        );
        return result.data;
      },
      ...
      byKey: async (key) => {
        let result = await $axios.get(
          options.loadUrl + loadToActionParams({ filter: filterByKey(key) })
        );
